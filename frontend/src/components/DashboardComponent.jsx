...
function DashboardComponent() {
  ...
  useEffect(() => {
    const fetchUsers = async () => {
      try {
        const { data } = await axios.get(`/api/users/getUsers?limit=5`);
        if (data.success) {
    ...
    const fetchUPosts = async () => {
      try {
        const { data } = await axios.get(`/api/posts/getPosts?limit=5`);
        if (data.success) {
    ...
    const fetchComments = async () => {
      try {
        const { data } = await axios.get(`/api/comments/getComments?limit=5`);
        if (data.success) {
